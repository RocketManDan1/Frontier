...
def angular_position_deg(elements: "OrbitElements", mu: float, time_s: float) -> float:
    """Return the angular position (degrees) of a body on its orbit at *time_s*.

    For circular orbits (e ≈ 0) this is simply ω + M.
    For eccentric orbits it uses the true anomaly: ω + ν.

    The result is normalised to [0, 360).
    """
    a = float(elements["a_km"])
    e = float(elements.get("e", 0.0))
    omega = math.radians(float(elements.get("omega_deg", 0.0)))
    M0 = math.radians(float(elements.get("M0_deg", 0.0)))
    epoch = float(elements.get("epoch_s", 0.0))
